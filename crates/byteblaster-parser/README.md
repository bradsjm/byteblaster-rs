...
```toml
[dependencies]
byteblaster-parser = { git = "https://github.com/bradsjm/byteblaster-rs", tag = "v0.2.0", package = "byteblaster-parser" }
```
...
```toml
[dependencies]
byteblaster-parser = { path = "../byteblaster-rs/crates/byteblaster-parser" }
```
...
```rust
use byteblaster_parser::{parse_text_product, TextProductHeader};
...
```rust
use byteblaster_parser::{parse_text_product, ParserError};
...
```bash
cargo test -p byteblaster-parser
```

Run specific test:

```bash
cargo test -p byteblaster-parser wmo_header_variations_parse
```

## Integration

This crate is used by `byteblaster-cli` for parsing weather products received via QBT or Weather Wire protocols.

See `byteblaster-cli/src/cmd/file_pipeline.rs` for usage examples in a real-world application.
