...
fn checksum_fixture() {
    ...
    assert!(matches!(events[0], QbtFrameEvent::DataBlock(_)));
}
...
fn stream_policy_behavior() {
    ...
    let good_checksum = calculate_qbt_checksum(&good_body) as u32;
    ...
    assert!(events.iter().any(|evt| {
        matches!(evt, QbtFrameEvent::DataBlock(segment) if segment.filename == "good.bin")
    }));
    assert!(!events.iter().any(
        |evt| matches!(evt, QbtFrameEvent::DataBlock(segment) if segment.filename == "bad.bin")
    ));
}
